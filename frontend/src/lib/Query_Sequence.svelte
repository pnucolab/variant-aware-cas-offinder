<script>
	...
	export let files;
	export let showFileWarning = false;
...
<div class="h-full w-full bg-white flex flex-col items-left">
    ...
    <Label class="space-y-1 mb-2">
        ...
        <Fileupload bind:files={files} on:upload={handleUpload}/>
            {#if showFileWarning}
            <Alert class="mt-2" color="red" dismissable on:close={() => showFileWarning = false}>
                <span class="font-medium">VCF file required!</span> Please upload a VCF file before submitting.
            </Alert>
            {/if}
            ...
            <div>
                ...
                <Textarea {...textareaprops} bind:value={query_seq} on:input={queryInput} on:paste={handlePaste} required />
